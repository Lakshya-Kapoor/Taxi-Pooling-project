...
function initialize(passport, getUserByRollNo, getUserById) {
  const authenticateUser = async (rollNo, password, done) => {
    ...
    try {
      // Because we're using await, we use try catch
  ...
  passport.use(
    new LocalStrategy({ usernameField: "rollNo" }, authenticateUser)
  ); // The username field in our login form is rollNo basically
